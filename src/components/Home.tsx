...
import { Layout, Menu, Avatar, Button, Table,Checkbox, Pagination, TablePaginationConfig, GetProp, TableProps } from "antd";
import {
  UserOutlined,
  LogoutOutlined,
  HomeOutlined,
...
const columns = [
  ...
  },
  {
    ...
    render: (riskSatisfaction: any) => (
      <span className={`flex items-center ${riskSatisfactionColor(riskSatisfaction)}`}> <div className={`w-3 h-3 rounded-full ${riskSatisfactionBgColor(riskSatisfaction)} mr-2`}></div> {riskSatisfaction}</span>
    ),
  },
  {
    title: 'Status',
    ...
    key: 'status',
    render: (status:any) => (
      <span className={`px-2 py-1 rounded ${statusBadgeColor(status)}`}>{status}</span>
    ),
  },
  {
    title: 'Result',
          ...
          >
            ...
            <div className="sub-head-section flex justify-between">
              <div className="left-sub-head flex">
                ...
                <span className="text-green-600 ml-2">DATA ENTRY</span>

                <span className="text-gray-400 ml-4">
                  <GiRadarSweep />
                </span>
                <span className="text-gray-400 ml-2">TRACKER</span>
              </div>
              ...
              <Link to={"/login"} className="ml-4">
                    <Button type="primary" className="bg-green-600 p-5 font-bold">Submit For Approval</Button>
                  </Link>
              </div>
            </div>
            <Table dataSource={dataSource} columns={columns}  pagination={tableParams.pagination} loading={loading}
      onChange={handleTableChange} rowSelection={Selection} className="mt-6"/>
